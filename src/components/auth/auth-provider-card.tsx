...
export default function AuthProviderCard({ 
  ...
  const providerConfig = {
    ...
    apple: {
      name: 'Apple',
      icon: (
        <svg width="20" height="20" viewBox="0 0 24 24" className="mr-3" fill="currentColor">
  ...
  return (
    <button
      onClick={enabled && !comingSoon ? onClick : undefined}
      disabled={isDisabled}
      className={`w-full flex items-center justify-center px-6 py-4 border border-gray-300 rounded-lg transition-all duration-200 ${
        isDisabled 
          ? 'opacity-50 cursor-not-allowed bg-gray-50' 
          : 'hover:bg-gray-50 hover:border-gray-400 hover:opacity-80 cursor-pointer bg-white'
      } ${className}`}
    >
      {config.icon}
      <span className={`font-medium ${isDisabled ? 'text-gray-400' : 'text-gray-700'}`}>
        {config.text}
        {comingSoon && <span className="ml-2 text-xs text-gray-400">{t('auth.comingSoon')}</span>}
      </span>
    </button>
  );
}